...
export const SplashScreen = (): React.ReactElement => {
  ...
  const sortedNegotiations = Object.values(negotiations).toSorted(
    (negotiationA: Negotiation, negotiationB: Negotiation) =>
      negotiationB.updatedAt - negotiationA.updatedAt,
  )

  const handleCreate = (): void => {
    const id = createNegotiation()
    navigate({ to: "/negotiation-maker/$negotiationId", params: { negotiationId: id } })
  }
    ...
    <div css={{ padding: spacing.xlarge, maxWidth: 900, margin: "0 auto" }}>
      ...
      {sortedNegotiations.length === 0 ? (
        <p css={{ color: colors.textDim, textAlign: "center", marginTop: spacing.xlarge }}>
          Click the New Negotiation button to create your first negotiation.
        </p>
      ) : (
        <ul css={{ listStyle: "none", padding: 0, margin: 0 }}>
          {sortedNegotiations.map((negotiation) => (
            ...
            >
              <button
                onClick={() => handleOpen(negotiation.id)}
                css={{
                  flex: 1,
                  textAlign: "left",
                  background: "none",
              ...
              <AlertDialog.Root>
                ...
                <AlertDialog.Portal>
                  ...
                  >
                    ...
                    >
                      This action cannot be undone. The negotiation "
                      {negotiation.title || "Untitled"}" will be permanently deleted.
                    </AlertDialog.Description>
      ...
      >
        ...
        >
          The negotiations you make on this site are automatically saved in your browser. None of
          your data is sent to anyone. This also means if you clear all your browser data your
          negotiations here will be gone, so download the files for them if you want to keep a
          backup.
        </p>
        <p
          css={{
